...
FROM THEMEPARK LEFT JOIN ATTRACTION
ON THEMEPARK.PARK_CODE = ATTRACTION.PARK_CODE;


SELECT THEMEPARK.PARK_CODE, PARK_NAME, ATTRACT_NAME
FROM THEMEPARK RIGHT JOIN ATTRACTION
ON THEMEPARK.PARK_CODE = ATTRACTION.PARK_CODE;

SELECT T.PARK_CODE, PARK_NAME, ATTRACT_NAME
FROM THEMEPARK T INNER JOIN ATTRACTION A
ON T.PARK_CODE = A.PARK_CODE;

-- DATE/TIME

SELECT EMP_FNAME, MONTH(EMP_DOB) AS "MONTH", YEAR(EMP_DOB) AS "YEAR" FROM EMPLOYEE;

SELECT current_date();

-- // lẤY RA NGƯỜI CÓ THÁNG SINH TRONG THÁNG NÀY TỪ BẢNG SAU 
SELECT * FROM EMPLOYEE where month(EMP_DOB) = month(CURRENT_DATE());

-- //  lẤY RA SỐ NGÀY EMP ĐÃ LÀM TỪ BẢNG EMP
-- DATAIFF(DAY, START DATE, END DATE)
select EMP_FNAME, DATEDIFF(CURRENT_DATE(), EMP_HIRE_DATE) FROM EMPLOYEE;

-- FORMAT
SELECT EMP_FNAME, DATE_FORMAT(EMP_DOB, '%d-%m-%y') AS Formatted_DOB
FROM EMPLOYEE;
...
SELECT concat(EMP_LNAME, ' ', EMP_FNAME) FROM EMPLOYEE;
SELECT EMP_FNAME, substr(EMP_FNAME, 2, 3) from EMPLOYEE;
SELECT length(EMP_FNAME) FROM EMPLOYEE;

-- lẤY THÔNG TIN CỦA NGƯỜI CÓ TUỔI TRẺ NHẤT TỪ BẢNG EMPLOYEE 
SELECT * FROM EMPLOYEE;

SELECT *, YEAR(CURRENT_DATE()) - YEAR(EMP_DOB) AS Age
FROM EMPLOYEE
ORDER BY EMP_DOB DESC
LIMIT 1;

SELECT * FROM EMPLOYEE WHERE EMP_DOB = (SELECT MAX(EMP_DOB) FROM EMPLOYEE);
...
SELECT T.EMP_NUM, EMP_FNAME, ATTRACT_NO FROM EMPLOYEE T cross JOIN  HOURS H ON T.EMP_NUM = H.EMP_NUM;
